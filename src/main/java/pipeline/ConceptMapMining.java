...
public class ConceptMapMining extends JCasConsumer_ImplBase {
	...
	public static final String PARAM_PRINT_MAPS = "printMaps";
	@ConfigurationParameter(name = PARAM_PRINT_MAPS, defaultValue = "false")
	...
	private void createTokenForSentence(Sentence sent, int sentId) {
		...
		for (Token token : JCasUtil.selectCovered(Token.class, sent)) {
			String key = meta.getDocumentId() + "-" + token.getAddress();
			PToken tok = new PToken(token, this.getSentKey(sent), i);
			this.tokMap.put(key, tok);
			i++;
		}
	}
	...
	public Extractor getPrevExtractor(CmmComponent requestor) {
	...
	private void log(String msg) {
		try {
			getContext().getLogger().log(Level.INFO, name + ": " + msg);
		} catch (UIMARuntimeException e) {
			Logger.getGlobal().log(java.util.logging.Level.INFO, name + ": " + msg);
		}
	}
